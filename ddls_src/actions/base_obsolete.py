...
class ActionIndex:
    ...
    def _index_single_action(self, action_tuple: Tuple, action_index: int):
        ...
        if not action_type.params: return
...
class SimulationAction(Enum):
    """
    This rich Enum is the single source of truth for all action-related information.
    It now includes a centralized method to look up an action by its integer ID.
    """
    ...
    def __init__(self,
        ...
        # Populate the class-level map during initialization
        self.__class__._id_map[id_val] = self

    _id_map = {}  # Class-level dictionary to store the ID-to-member mapping
    ...
    def get_by_id(cls, action_id: int):
        """
        A centralized, robust method to get an enum member from its integer ID.
        """
