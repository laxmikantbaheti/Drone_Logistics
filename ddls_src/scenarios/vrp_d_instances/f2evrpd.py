...
class F2EVRPDGenerator(BaseDataGenerator):
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Required
        # ------------------------------------------------------------------
        self.instance_stub = config.get("instance_path")
        if not self.instance_stub:
        ...
        try:
            if not os.path.isabs(self.instance_stub):
                here = os.path.dirname(os.path.abspath(__file__))
                self.instance_path = os.path.abspath(os.path.join(here, self.instance_stub))
            else:
                self.instance_path = self.instance_stub+".vrp"
                self.mh_path = self.instance_stub+".mh"
                self.dist_path = self.instance_stub+".dist"
        ...
        self.drones_per_mh = int(config.get("drones_per_mh", 1))
    ...
    def generate_data(self) -> Dict[str, Any]:
        ...
        parsed_mh = self._parse_microhubs(self.mh_path)
        ...
        # Micro hubs
        self.num_microhubs = parsed_mh["num_microhubs"]
        micro_hubs_list = self._build_micro_hubs(parsed_mh, nodes_list)

        self.num_drones = self.drones_per_mh * len(micro_hubs_list)
        ...
        drones_list = self._build_drones(self.num_drones, depot_id, drone_config, micro_hubs_list)
    ...
    def _parse_microhubs(self, path: str) -> Dict[str, Any]:
        name = None
        num_microhubs = None
        ...
        with open(path, "r", encoding="utf-8", errors="ignore") as f:
            for raw in f:
                ...
                u = line.upper()
                if u.startswith("MICROHUB_COORD_SECTION"):
                    mode = "COORDS"
                    continue
                if u.startswith("EOF"):
                    break
                ...
                if mode is None and ":" in line:
                    ...
                    if k.upper() == "NAME":
                        name = v
                    elif k.upper() == "NUM_MICROHUBS":
                        num_microhubs = int(float(v))
                    continue
        ...
        if num_microhubs is None:
            raise ValueError("NUM_MICROHUBS missing in microhubs file.")

        return {
            "name": name,
            "num_microhubs": num_microhubs,
            "coords": coords,
        }
    ...
    def _build_micro_hubs(self, parsed_mh, nodes_list):
        hubs = []
        max_id = max(n["id"] for n in nodes_list)
        for i in range(self.num_microhubs):
            hubs.append({
                "id": max_id + len(hubs) + 1,
                "coords": list(parsed_mh["coords"]["MH"+"_"+str(i+1)]),
                "type": "micro_hub",
